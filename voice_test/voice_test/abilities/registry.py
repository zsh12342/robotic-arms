...
class Ability(pydantic.BaseModel):
    ...
    def __str__(self) -> str:
        ...
        func_summary += f",Usage: {self.description};"
        return func_summary
...
def ability(
    ...
    def decorator(func):
        ...
        # param_names.add("agent_map")
        # param_names.add("task_id")
        # param_names.add("configs")
...
class AbilityRegister:
    def __init__(self, configs, agent_map=None) -> None:
        self.abilities = {}
                ...
                try:
                    ...
                    for attr in dir(module):
                        ...
                        if hasattr(func, "ability"):
                            ...
                            if func.ability.name in self.configs:
                                self.abilities[func.ability.name] = func.ability
                except Exception as e:
                    print(f"Error occurred while registering abilities {str(ability)}: {str(e)}")

    def list_abilities(self) -> List[Ability]:
        return self.abilities

    def list_abilities_for_prompt(self) -> List[Dict[str, Any]]:
        return [{"name": ability.name, "description": ability.description} for ability in self.abilities.values()]
    ...
    async def run_ability(
        ...
        try:
            if ability_name in self.abilities:
                ...
                return await ability(*args, **kwds)
            else:
                print(f"warning: {ability_name} not exist")
        except Exception:
            raise
