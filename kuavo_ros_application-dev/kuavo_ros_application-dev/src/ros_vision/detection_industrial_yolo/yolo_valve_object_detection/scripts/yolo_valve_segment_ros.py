...
import os
import sys
sys.path.append(os.path.join(rospkg.RosPack().get_path('yolo_valve_object_detection'), 'scripts'))
...
config_path = os.path.join(rospack.get_path('yolo_valve_object_detection'), 'config/yolo.yaml')
...
def broadcast_tf_transforms(detection_msg, tf_broadcaster):
    for detection in detection_msg.detections:
        if detection.results:
            ...
            transform.child_frame_id = f"valve_object_{detection.results[0].id}"  # 使用检测到的对象 ID 作为子帧 ID
...
# 主函数
def main():
    rospy.init_node('yolo_valve_detection_node')

    # 创建发布者
    pub = rospy.Publisher('/object_yolo_valve_segment_result', Detection2DArray, queue_size=10)
    image_pub = rospy.Publisher('/object_yolo_valve_segment_image', Image, queue_size=10)
    mask_pub = rospy.Publisher('/object_yolo_valve_segment_mask', Image, queue_size=10)  # 新增mask发布者
    ...
    # 初始化YOLOSeg对象
    model_path = os.path.join(rospkg.RosPack().get_path('yolo_valve_object_detection'), 'scripts/models/yolo5s_forValve.onnx')
