...
class YoloTransform:
    def __init__(self):
        # 初始化节点
        rospy.init_node('yolo_valve_transform', anonymous=True)

        # 订阅检测结果话题
        self.detection_sub = rospy.Subscriber('/object_yolo_valve_segment_result', Detection2DArray, self.detection_callback)

        # 发布转换后的结果话题
        self.transformed_pub = rospy.Publisher('/object_yolo_valve_tf2_torso_result', Detection2DArray, queue_size=10)
    ...
    def detection_callback(self, msg):
        ...
        for detection in msg.detections:
            ...
            transform_stamped.child_frame_id = f"torso_valve_object_{detection.results[0].id}"
